...
**단계**: R1 실환경 배포 완료 → Bolt 4 안전 재시작(FR-CO-02) → Fleet(FR-FL-01/03) → 리소스 그룹 db 전환(D-010) 사내 적용 완료 + 편집 화면(FR-WL-07~10) → 벤치마크 하네스(FR-BM-01/03/04) + 쿼리 세트 화면 관리(FR-BM-06, D-014) → **React SPA 전환 완료 (D-016, 화면 12/12, `src/tms/web/` 삭제)**

**사내 미적용분**: 마이그레이션 `020`/`021` (벤치마크 스케줄, D-017). `010`~`019` 는 전량 적용됐다 (2026-08-26). 순서와 게이트는 `docs/TODO.md`.
