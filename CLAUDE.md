...
| Python | 3.9+ 호환 필수 |
| 백엔드 | FastAPI + systemd |
| 프론트 | **React 19 SPA 로 전환 중** (Vite 정적 빌드 → FastAPI 서빙, **런타임 Node 없음**). D-016, 2026-08-26. 인증은 **서버가 설정하는 세션 쿠키 유지** — `localStorage` 토큰으로 바꾸지 않는다. 현재 화면은 아직 Jinja(`src/tms/web/`)이며 API 확충이 선행이다 |
...
**단계**: R1 실환경 배포 완료 → Bolt 4 안전 재시작(FR-CO-02) → Fleet(FR-FL-01/03) → 리소스 그룹 db 전환(D-010) 사내 적용 완료 + 편집 화면(FR-WL-07~10) → **벤치마크 하네스(FR-BM-01/03/04) + 쿼리 세트 화면 관리(FR-BM-06, D-014)**

**사내 미적용분**: 마이그레이션 `010`~`019`. 순서와 게이트는 `docs/runbooks/onsite-checklist.md`.

### ⛔ 프론트 전환 대기 중 — 지금 무엇을 만들고 무엇을 만들지 않는가

D-016 으로 **`src/tms/web/` 는 React SPA 로 대체된다.** 착수 전까지:

| 만든다 | 만들지 않는다 |
|---|---|
| **JSON API** — 어떤 프론트에서도 필요하고, 화면보다 오래 산다 | **새 Jinja 화면** |
| 서비스·저장소 계층의 규칙과 그 테스트 | 기존 화면의 다듬기·리팩터 |
| 사내 검증 (마이그레이션·서버 동작) | htmx 상호작용 추가 |

**사내 검증(V-8·V-9)은 그대로 진행한다.** 확인 대상의 대부분이 서버 쪽이고 — 허용 구문 검사, 감사 기록, SQL 스냅샷, 실행 조건 기록 — 전환 후에도 그대로 산다. **다만 화면 자체를 다듬는 데는 시간을 쓰지 않는다.**

착수 조건은 `W-9`(화면 12개 × 필요한 API 목록화)와 사내 마이그레이션 `018`/`019` 적용이다.
