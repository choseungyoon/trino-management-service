...
| 구분 | 개수 | 성격 |
|---|---|---|
| **결정** (D) | **2건** (D-1·D-3 해소, 2026-08-21) | 답이 나와야 코드가 움직인다 |
| **확인** (V) | 7건 | 사내에서 눈으로 봐야 아는 것 |
| **작업** (W) | 7건 | 절차·설정·타 팀 협조 |

**지금 가장 값이 큰 것 3개**
1. **V-10 콘솔이 뜨는가** — 화면이 통째로 바뀌었고 사내 실데이터로는 처음이다. 나머지 전부의 전제
...
### ~~⭐ D-1. TMS 에 `ExecuteQuery` 권한을 줄 것인가~~ — ✅ **해소 (2026-08-21): 부여 (D-012)**
...
- [x] **부여한다 (D-012).** FR-FL-02 하나만이었다면 부여하지 않았을 것이다 — 결정을 뒤집은 것은 FR-BM-01 이 같은 권한에 막혀 있었다는 사실이다. 절차는 `runbooks/executequery-grant.md`, 강제 수단은 `tests/test_sql_isolation.py` 로 옮겼다
...
### ~~D-3. R2 범위 문서 불일치 (H1)~~ — ✅ **해소 (2026-08-21): 부록 B 기준**
...
- [x] 기준 문서 확정 → `BACKLOG.md` 를 부록 B 에 맞췄다. FR-BENCHMARK 는 **BM-01·03·04 로 범위를 못 박았다** — BM-02(Prometheus)·BM-05(히스토리 통합)는 이 릴리스에서 불가능하다
...
| 후보 | 상태 | 막는 것 |
|---|---|---|
| **FR-CO-01** 설정 조회·변경 | 착수 가능 | 없음 (Ansible 실행기 재사용). **다음 슬라이스 후보 1순위** |
| ~~**FR-FL-04/05**~~ | ✅ **2026-08-15 구현 완료** | — |
| **FR-FLEET-DRIFT** config 체크섬 | 부분 | 노드별 체크섬 수집용 새 플레이북 필요 |
| FR-FL-02 미조인 워커 식별 | 막힘 | **D-1** |
| ~~FR-GW-04~~ | ⛔ **미충족 확정 (2026-08-15)** — Gateway 가 캐시 적중 신호를 내지 않는다. 화면은 결과만 말한다 | Gateway 쪽 엔드포인트/메트릭 |
| FR-SLO | 막힘 | 목표값(인간 결정) + 워크로드 데이터 |
| FR-CATALOG | 보류 | `catalog.management=dynamic` (experimental) 도입 결정 |

~~**권고: FR-FL-04/05.**~~ **2026-08-15 구현 완료.** 설정된 플레이북을 `reason` 과 함께 실행하고, 출력을 스트리밍하며, 이력을 남긴다. `fleet.jobs` 가 비어 있으면 화면에 나타나지 않는다.

**다음 권고: FR-CO-01(설정 조회·변경).** 같은 실행기를 쓰고, "지금 클러스터 설정이 무엇인가"는 여전히 SSH 로 들어가야만 알 수 있다. 다만 조회는 노드별 파일을 읽어오는 새 플레이북이 필요하다 — 그 플레이북은 사람이 써야 한다.
...
### 🔴 V-10. 콘솔이 뜨는가 `[다른 무엇보다 먼저]`

**2026-08-27 에 화면이 통째로 바뀌었다** (D-016). 서버 렌더 Jinja 콘솔은
삭제됐고, `/` 는 커밋된 React 번들이다. 사내 실데이터로는 **한 번도 돌려
본 적이 없다.**

D-016 이 명시적으로 포기한 것이 여기서 처음 현실이 된다: **번들이 못 뜨면
빈 화면이다.** 서버 렌더에는 없던 실패 모드이므로, 다른 확인을 하기 전에
이것부터 본다.

- [ ] `systemctl restart tms-api` 후 브라우저로 `/` → 로그인 화면이 그려지는가
      <br>*빈 화면이면 개발자 도구 Network 에서 `/static/index-*.js` 가 404 인지 본다 —
      404 면 `pip install` 이 번들을 안 넣은 것이다 (`deploy.md` §UI 파일 문제)*
- [ ] 로그인 → Overview 가 그려지는가
- [ ] 왼쪽 내비게이션의 12개 항목이 각각 열리는가 (하나라도 빈 화면이면 거기서 멈춘다)
- [ ] `curl -sk https://<host>:8500/health` → `{"status":"ok"}`
      <br>*⛔ 이건 프로브지 화면이 아니다. 헬스 **화면**은 `/cluster-health` 다*

**주소가 바뀐 것들** — 북마크와 사내 위키를 고쳐야 한다:

| 전 | 후 |
|---|---|
| `/clusters/<c>/health` | `/cluster-health?cluster=<c>` |
...
**되돌리는 방법**: `git checkout f3cec7a~1` 이 컷오버 직전이다. 서버 렌더
콘솔이 그대로 있고 React 콘솔은 `/app` 에 있다. 되돌려야 했다면 그 이유가
D-016 의 "뒤집는 조건 1" 이다 — 기록에 남긴다.

### ⭐ V-1. 배포 후 5개 화면 `[내일]`

> **V-10 이 선행이다.** 화면이 아예 안 뜨면 여기 항목은 전부 같은 원인이다.
...
`runbooks/onsite-checklist.md` §8. **게이트는 2026-08-26 (D-015) 에 라벨로 바뀌었다** —
운영 중인 클러스터에서도 실행된다. 확인할 것은 "거부되는가" 가 아니라 **조건이 숫자를
따라다니는가**다.

- [ ] 운영 중인 클러스터가 **`Serving traffic`** 으로 표시되고 이유가 문장으로 나온다.
      체크박스는 잠기지 않는다
- [ ] 실행 → 실행 화면에 노란 배너, 목록의 `Cluster was` 열이 `Serving traffic`
- [ ] 감사 로그에 `BENCHMARK_RUN` 이 사유와 함께 남는다
- [ ] 같은 클러스터에 하나 더 실행 시도 → **거부된다** (두 실행이 서로를 측정한다)
- [ ] 사람이 제외한 뒤 `Quiet` → 실행 → 쿼리별 중앙값이 채워진다
- [ ] ⛔ `Quiet` 실행과 `Serving traffic` 실행을 비교 → **조건이 다르다는 경고**.
      이게 게이트를 대신하는 안전장치다
- [ ] `tms_app` 으로 `UPDATE benchmark_result` / `DELETE` → **둘 다 실패**

> ⚠️ **무거운 세트를 운영 클러스터에 돌리는 것은 그 자체로 부하다.** 가벼운 세트 ·
> 반복 1회로 시작한다. 재려던 느려짐을 스스로 만들 수 있다.

**쿼리 세트는 화면에서 만든다** (FR-BM-06 · D-014, 2026-08-25). 설정 파일에 `benchmark.query_sets`
가 남아 있으면 기동이 거부된다 — 무시하면 두 개의 진실이 되기 때문이다. 세트가 Git 밖으로
나갔으므로 아래도 같이 본다:

- [ ] `DELETE ...` 을 저장해 본다 → **거부**. 주석으로 가려도 → **거부**
- [ ] 사유 없이 저장 → **거부**. 감사에 `BENCHMARK_QUERY_CHANGE` 가 남는다
- [ ] 쿼리를 고친 뒤 이전 실행을 다시 연다 → **그 실행이 쓴 SQL 은 그대로다**
- [ ] 고치기 전후 실행을 비교 → 해당 행에 **변경됨** 표시
- [ ] 실행 중인 세트를 고쳐 본다 → **거부**
...
다음 배포 즉시 🔴 V-10 콘솔이 뜨는가 — 화면이 통째로 바뀌었다 (D-016)
