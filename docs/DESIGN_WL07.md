...
**이 원자성이 D-010 에서 TMS DB 를 고른 이유 그 자체다.** 리소스 그룹과 감사가 같은 database 에 있어서 한 트랜잭션에 들어간다 — "값은 바뀌었는데 감사 기록은 없는" 상태가 성립하지 않는다.
...
| | 질문 | 권고 |
|---|---|---|
| **H-1** | TMS 전용 DB 계정에 `trino_resource_groups` schema 쓰기 권한을 줄 것인가 | **2단계 착수 전에 필요하다.** 지금은 코디네이터용 읽기 계정만 있다. TMS 쓰기 계정을 따로 만들어야 "누가 바꿨나"를 계정 수준에서도 답할 수 있다 |
