...
>
> **⛔ 교차 검증에는 신선도 상한이 필수다 (2026-08-07 실측으로 발견한 버그).** 컬렉터는 한 tick 안에서 **쿼리를 JMX보다 먼저** 폴링하므로, `RunningQueries` 는 항상 저장소에 있던 **이전** JMX 스냅샷에서 읽는다. 재기동 직후 첫 tick 에서는 그 스냅샷이 몇 시간 전 것일 수 있고, 실제로 유휴 클러스터에 H-09 오탐이 발생했다. **JMX 스냅샷이 `jmx_interval × 3` 보다 오래되었으면 교차 검증을 건너뛴다** (`ClusterPoller.jmx_cross_check_max_age`). 오탐은 이 경보의 신뢰도를 무너뜨리며, 신뢰받지 못하는 경보는 진짜 사일런트 필터링을 놓친다.
