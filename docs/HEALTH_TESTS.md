...
>
> **⛔ 오탐 방지 3중 장치 (2026-08-07 실환경 UI 구동 중 발견).** 두 값은 **원자적으로 읽히지 않는다.** 이 사실을 무시하면 정상 클러스터에서 H-09 가 `GOOD ↔ UNKNOWN` 으로 계속 깜빡인다(실측).
>
> | # | 장치 | 이유 |
> |---|---|---|
> | 1 | **폴링 순서: JMX → 쿼리** (`ClusterPoller._POLL_ORDER`) | 원래는 쿼리가 먼저였다. 그 경우 `RunningQueries` 는 최대 `jmx_interval`(10~15초) 이전 값이라 편차가 크다. 순서를 바꾸면 두 값의 간격이 HTTP 왕복 1회로 줄어든다 |
> | 2 | **신선도 상한** (`jmx_cross_check_max_age`, 기본 `jmx_interval × 3`) | 재기동 직후 첫 tick 이 몇 시간 전 스냅샷과 비교하는 것을 막는다. 실제로 유휴 클러스터에 오탐 발생 |
> | 3 | **연속 확인** (`h09_confirm_polls`, 기본 2회) | 1·2 를 적용해도 두 읽기 사이에 쿼리가 종료되면 필터링과 구별되지 않는다. **실측: 3.5분간 오탐 3회.** 레이스는 다음 폴에서 해소되지만 **권한 거부는 절대 해소되지 않는다** — 지속성이 곧 판별식이다 |
>
> 대가는 진짜 필터링 탐지가 **폴 주기 1회(3~5초) 늦어지는 것**뿐이다. 권한 오설정은 사람이 고치기 전까지 지속되므로 이 지연은 무의미하다. 반대로 오탐은 이 경보의 신뢰도를 무너뜨리고, 신뢰받지 못하는 경보는 진짜 사일런트 필터링을 놓친다.
...
>
> **계획된 drain을 장애로 오판하지 않는다 (신규 판정 규칙)**
> `DrainingNodeCount` 또는 `ShuttingDownNodeCount` 가 0보다 크면, 그만큼은 **의도된 축소**일 수 있다. 판정식을 다음으로 보정한다.
>
> ```
> unavailable = expected - ActiveNodeCount
> planned     = DrainingNodeCount + DrainedNodeCount + ShuttingDownNodeCount
> unplanned   = max(0, unavailable - planned)
> ```
...
| 항목 | 값 |
|---|---|
| 소스 | `FailedQueries.FiveMinute.Count` / **`CompletedQueries.FiveMinute.Count`** (§T3-7) |
...
>
> **⛔ 분모는 `StartedQueries` 가 아니다 (2026-08-07 실측으로 발견한 버그).** 실행 전에 거부된 쿼리는 `Failed` 와 `Completed` 를 올리지만 `Started` 는 올리지 않는다. `failed/started` 로 계산하면 100%를 넘고, 실제로 UI에 **`120.5%`** 가 표시됐다. 하필 카탈로그 오설정·권한 거부처럼 "실행 전 실패"가 몰리는 상황에서 오차가 커진다 — 헬스 체크가 가장 필요한 순간이다. 근거와 실측 델타는 `TRINO_VERIFIED.md` §T3-7. 구현은 분모를 `CompletedQueries` 로 쓰고, 그럼에도 100%를 넘으면 **경고 로그 후 100%로 클램프**한다. 불가능한 숫자를 화면에 띄우는 순간 나머지 지표의 신뢰도까지 같이 잃는다.
