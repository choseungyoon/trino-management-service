...
| **V1-4** | ⚠️ **미결론** — 클러스터가 유휴(0건)여서 `queries:view` 가 검증되지 않았다. **유휴 클러스터와 조용한 필터링은 구별 불가** |
| **V1-5** | ⚠️ **미결론** — 빈 목록 2 bytes. 피크 응답 크기 미측정 → 폴링 주기 미확정 |
| V1-7 | ✅ `/metrics` 접근 가능 |

> **V1-4/V1-5는 "쿼리가 실제로 실행 중일 때" 재실행해야 닫힌다.** 스크립트가 이 경우 `OK` 가 아니라 **`INCONCLUSIVE` 경고**를 내도록 수정했다 — 유휴 상태의 통과를 검증으로 오인하지 않기 위함이다.
> **V2~V11 구현은 이것 때문에 막히지 않는다** (§B2-5 참조).
