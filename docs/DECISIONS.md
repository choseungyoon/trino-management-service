...
**⛔ 선행 확인 (로컬 Trino 477 로 실측) — 미검증 2건**

| 확인할 것 | 왜 막는가 |
|---|---|
| `?currentSchema=` 로 지정한 schema 에 Trino 가 테이블을 **자동 생성하는가** | `public` 에 만들면 격리가 성립하지 않는다. 별도 database 로 가야 한다 |
| DB 가 죽어 있을 때 코디네이터가 **cold start 되는가** | `max-refresh-interval` 은 *돌고 있는* 코디네이터의 유예다. 기동 시점의 동작은 문서에 없다. 안 되면 "TMS DB 정지 중에는 클러스터를 재시작할 수 없다"는 운영 제약이 생기고, 안전 시퀀스 런북에 명시해야 한다 |
