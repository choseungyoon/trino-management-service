...
| **결정** (D) | 3건 | 답이 나와야 코드가 움직인다. **사내에 안 들어가도 된다** |
...
기본값은 `manual` 이고, 켜는 것은 별개의 행위다. `manual` 로는 **이미 한 번 완주했다** (2026-08-11).
게이트(빈 클러스터 확인 → 헬스 확인)는 두 모드가 완전히 동일하다.

- [ ] `cluster_ops.restart_mode: ansible` 전환 여부 결정
- [ ] 전환 시: TMS 호스트가 **전 Trino 노드에 SSH 접근**을 갖는다는 점을 보안 담당과 공유했는가
- [ ] 켜기 전 준비: Ansible 설치 · `binary` 절대경로 · SSH 키는 `/etc/tms/ssh/`
      (`ProtectHome=true` 라 `/home/tms/.ssh` 는 못 읽는다) · 유닛 재배포로
      `StateDirectory=trino-management-service` 반영 · `tms-config-check`
      <br>*절차: `upgrade-r2-r3.md` §4-3-1*

> **같은 질문이 하나 더 있다**: Fleet 작업 실행(`fleet.jobs`)도 켜면 같은 SSH 접근이 생긴다.
> ⛔ **재시작하는 플레이북은 거기에 넣지 않는다** — 드레인 검사가 없으므로 절대규칙 5 를
> 우회하는 경로다. `tms-config-check` 가 이 경우를 거부한다.

### D-4. 다음 개발 슬라이스

| 후보 | 상태 | 막는 것 |
|---|---|---|
| **FR-CO-01** 설정 조회·변경 | 착수 가능 | 없음 (Ansible 실행기 재사용). **1순위** |
...
**권고: FR-CO-01.** "지금 클러스터 설정이 무엇인가"는 여전히 SSH 로 들어가야만 알 수 있다.
다만 조회는 노드별 파일을 읽어오는 새 플레이북이 필요하고, 그 플레이북은 사람이 써야 한다.
...
- [ ] `D-2` 는 그대로 needs_decision · `D-5`(컷오버 유지) 를 새로 올린다
...
```
사내 들어가면  🔴 V-10 콘솔이 뜨는가        ← 게이트. 막히면 여기서 끝
               V-2 / V-3  (Fleet · Workload 가 실제로 보는가)
               V-9 리소스 그룹 편집 · V-8 벤치마크 나머지
               W-1 실측 [피크 시간대]  ← R1 DoD 가 닫힌다
               §1-9 끝나고 · §4 보드 정리

사외에서       D-5 컷오버 유지 여부 (V-10 결과를 보고)
               D-2 ansible 전환 여부 · D-4 다음 슬라이스
               → 셋 다 회의 없이 답할 수 있다
