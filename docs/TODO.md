...
> **⚠️ 2026-08-08 재검토 — 현재 사용자 약 50명이고 아직 운영 서비스가 아니다.**
> 아래 우선순위는 5만 규모를 전제로 잡은 것이다. 실제로는 **지금이 가장 바꾸기 쉬운 시기**이므로,
> "위험하니 나중에"가 아니라 **"쉬울 때 미리"** 로 읽는 편이 맞다. 특히 C-1(SPOF)과 B-2(라우팅)는
> 사용자가 늘기 전에 끝내두는 것이 훨씬 싸다.

### B-1. Gateway API 계정 `[요청 완료, 회신 대기]`

> **대안 진행 중**: 로컬에 Gateway 19 를 직접 설치해 필요한 설정을 전부 검증한 뒤,
> "이렇게 설정해달라"는 구체적 요청서를 만드는 방식으로 전환했다. 회신을 기다리지 않아도 된다.
