...
| # | 원칙 | 근거 |
|---|---|---|
| **A1** | **TMS는 폴링 경로에서 SQL 쿼리를 제출하지 않는다.** 수집기는 REST(`/v1/*`)와 JMX-over-HTTP(`/v1/jmx/*`)만 쓴다. **개정 2026-08-21 (D-012)** — 사용자가 시작한 저빈도 조회는 예외이며 `clients/sql.py` 한 곳에서만 나간다 | 아래 §1-1 |
...
> **✅ 2026-08-21 — 그 판단을 했다 (D-012).** `tms-svc` 에 `ExecuteQuery` 를 부여했다.
>
> **위 세 가지 비용은 전부 빈도의 함수였다.** FR-FL-02 는 코디네이터의 노드 개수와 인벤토리가 어긋났을 때만 한 번 묻고, 평상시 쿼리 수는 0 이다. FR-BM-01 은 라우팅 그룹에서 뺀 클러스터에서만 돈다 (FR-BM-04, 타협 불가).
>
> **A1 은 사라지지 않고 좁아진다.** 강제 수단이 `execute` 권한 부재에서 **코드**로 옮겨갔다:
> - SQL 은 `clients/sql.py` 한 곳에서만 나간다
> - `tms.collector.*` 가 그 모듈을 import 하면 **테스트가 실패한다**. 폴링 경로에 SQL 이 들어가는 것을 사람의 기억이 아니라 CI 가 막는다
