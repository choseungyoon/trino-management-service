...
| FR-WL-07 | **리소스 그룹 설정 트리 조회** (DB 소스) + 실행 중 상태(JMX)와 대조. **2026-08-14 설계** — `DESIGN_WL07.md` | 설정된 전체 목록 표시 |
| FR-WL-08 | 그룹 **값 수정** — `reason` + 감사 + admin. 10초 내 반영, 재시작 없음 | 저장 후 반영 확인 |
| FR-WL-09 | 그룹·셀렉터 **추가/삭제**. ⛔ 삭제는 `ON DELETE CASCADE` 파급을 실행 전에 나열 | 파급 목록 표시 |
| FR-WL-10 | 변경 **이력 + 되돌리기**. Trino 테이블에 이력이 없어 TMS 가 별도 보관 | 되돌리기 동작 |
