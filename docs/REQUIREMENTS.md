...
| FR-FL-02 | 워커 등록 상태 및 discovery 조인 여부 | ✅ **구현 완료 (2026-08-21, D-012)** — `system.runtime.nodes` 를 **요청 시에만** 조회해 미조인 노드를 이름으로 짚는다. 폴링하지 않는다 |
...
~~**미충족 AC**: FR-FL-02 는 `ExecuteQuery` 없이 불가능하다.~~

**✅ 2026-08-21 해소 (D-012).** `ExecuteQuery` 를 부여했다. 다만 **폴링하지 않는다** — 코디네이터의 `ActiveNodeCount` 가 인벤토리보다 적을 때만 화면에 "코디네이터에게 물어보기" 가 나타나고, 누르면 `system.runtime.nodes` 를 한 번 조회한다. 평상시 쿼리 수는 0 이다. 그 조건이 D-012 의 전제이며, `tests/test_sql_isolation.py` 가 수집기의 SQL 접근을 막아 지킨다.

**반대 방향도 본다**: 인벤토리에 없는데 쿼리를 서빙 중인 노드도 표시한다. 더 드물고 더 놀라운 상태다.
...
| ID       | 내용                                          | AC               |
| -------- | --------------------------------------------- | ---------------- |
| FR-FD-01 | 노드별 config 체크섬 수집 및 기준값 대비 비교 | ⏸ **막힘** — 체크섬 수집 플레이북이 선행이고, 그것은 사람이 쓴다 |
| FR-FD-02 | 설정 변경 후 미재시작(stale) 노드 표시 | ⏸ **막힘** — FD-01 선행 |
| FR-FD-03 | 골든 이미지 버전 및 Trino 버전 불일치 탐지 | ✅ **구현 완료 (Fleet 수집기)** — Trino 버전 혼재 · `node.environment` 불일치 · 코디네이터 중복 주장을 노드 목록 위에 경고로 띄운다. 골든 이미지 버전은 인벤토리에 없어 미포함 |
...
### FR-BOARD (P1) — 작업 보드 (**TMS 고유**)

> **근거는 D-013 에 있다.** 여기에는 무엇을 만들었는지만 적는다.
> 보드는 **상태**의 주인이고, 각 항목이 가리키는 문서가 **근거**의 주인이다. 어긋나면 문서가 이긴다.

| ID       | 내용                                                                 | AC |
| -------- | -------------------------------------------------------------------- | -- |
| FR-BD-01 | 상태별 보드 조회 (결정 대기 / 차단 / 진행 중 / 예정 / 완료 / 폐기)   | 로그인한 모든 사용자가 본다. 빈 칸도 칸으로 남는다 |
| FR-BD-02 | 관리자가 **요청**을 올린다 (`REQ-n` 자동 부여)                       | 화면에서 만들 수 있는 종류는 `request` 뿐 |
| FR-BD-03 | 항목별 댓글 (추가 전용)                                              | 누가·언제가 남는다. 수정·삭제 없음 |
| FR-BD-04 | 상태 변경 + 변경 이력                                                | 누가 어디서 어디로 옮겼는지 별도 행으로 남는다 |
| FR-BD-05 | `docs/WORK_BOARD.md` 내보내기 (`tms-work-export`, `/work.md`)        | 사외에서도 읽을 수 있다 — 이것이 "착수 전 참조"를 실행 가능하게 만든다 |

**⛔ 만들지 않는 것**: 담당자 배정·기한·스프린트·워크플로 커스터마이즈. Jira 를 다시 만드는 것이 목적이 아니다 — **관리자가 요청을 넣을 곳과, 무엇이 사람을 기다리는지 보이는 것**이 전부다.

**쓰기 권한**: admin 한정. `reason` 필드와 감사 행은 두지 않는다 — 절대규칙 3 을 여기에 적용하지 않는 이유는 D-013 에 적혀 있다.
...
| FR-BM-04 | **실행 조건 기록** — 클러스터가 라우팅에서 빠져 있었는지·비어 있었는지 | 실행 행에 기록 + 화면 표시 + 조건 불일치 비교 시 경고 |
| FR-BM-05 | 실제 프로덕션 쿼리 샘플 기반 세트 생성 (EventListener 데이터 활용)   | 샘플 추출 동작      |
| FR-BM-06 | **쿼리 세트를 화면에서 관리** — 등록·수정·삭제 + 쿼리별 실행 이력 조회 | 배포 없이 반영, 쓰기마다 감사 |
| FR-BM-07 | **주기 실행 스케줄** — 세트·클러스터·주기를 화면에서 정하고 무인 실행 | 사람 없이 실행되며 actor/reason 이 감사에 남는다 · 연속 실패 시 자동 정지 |

**FR-BM-04 는 2026-08-26 에 게이트에서 라벨로 바뀌었다 (D-015).** 원래 문구는 "타협 불가 — 미제외 시 실행 거부" 였다. 뒤집힌 이유는 위험 판단이 바뀌어서가 아니라 **용도가 달라서**다: 상정했던 것은 "A 가 B 보다 왜 느린가" 를 파고드는 일회성 조사였고, 실제로 필요한 것은 운영 중인 클러스터의 성능을 주기적으로 재는 것이다. 게이트가 있으면 사람들이 실제로 쓰는 상태는 영원히 못 잰다.

**⛔ 게이트가 지키던 위험은 그대로다.** 무거운 세트는 같은 워커를 놓고 운영 쿼리와 경쟁하고, 자기가 재려던 느려짐을 자기가 만들 수 있다. 그래서 강제 대신 **숫자를 따라다니게** 했다 — `benchmark_run.guard` 에 기록, 화면에 `Quiet` / `Serving traffic` 표시, **두 실행의 조건이 다를 때** 비교에서 경고. 한 클러스터 동시 실행 거부는 남아 있다(두 실행이 서로를 측정한다).

**FR-BM-07 은 D-015 가 열어 놓고 만들지 않은 절반이다** (D-017, 2026-08-27). D-015 는 "주기적으로 재기 위해" 게이트를 없앴지만 주기 실행 수단은 만들지 않아서, 추세 차트의 모든 점이 사람이 버튼을 눌러야 생겼다. ⛔ **사람이 없는 시각에 운영 클러스터에 쓰는 유일한 기능이므로** actor·reason 은 스케줄에서 나오고(`created_by`/`reason`), 스케줄 편집은 자체 감사 액션(`BENCHMARK_SCHEDULE_CHANGE`)을 갖는다 — 스케줄은 **미래의 무인 실행을 승인**하는 행위다. cron 표현식은 쓰지 않는다.

**FR-BM-06 은 FR-BM-01 의 저장 위치를 바꾼다** (D-014, 2026-08-25). 세트는 `benchmark.query_sets` 설정이 아니라 DB 에 있고, `/benchmark/sets` 에서 편집한다. 함께 따라오는 제약 세 가지:

- **읽기 전용 구문만 저장된다.** 저장 시점과 실행 직전에 각각 검사한다 — 후자는 중복이 아니다. 행은 psql 로도 들어올 수 있다
- **결과 행은 표시하지 않는다.** 이것이 비목표(웹 SQL 에디터)와의 경계다. 경계는 입력이 아니라 출력에 있다
- **실행 시작 시점의 SQL 을 `benchmark_run.queries` 에 스냅샷한다.** 편집 가능한 세트에서는 키가 무엇이 실행됐는지를 말해 주지 못하므로, FR-BM-03 비교는 어긋난 쿼리에 표시를 단다
