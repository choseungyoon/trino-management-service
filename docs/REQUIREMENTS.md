...
0. **Bolt 0 (2026-08-04) 검증 결과가 반영되어 있다.** 취소선(~~`[VERIFY]`~~)이 그어진 항목은 해소된 것이며, 그 아래 "✅ Bolt 0 해소" 블록이 확정된 사실이다. **모든 사실의 출처는 `docs/TRINO_VERIFIED.md`이며, 판정 근거는 `docs/BOLT_0_RESULT.md`다.**
...
### FR-QUERY-HISTORY (P0) — 통합 쿼리 히스토리/감사
...
| FR-CH-06 | **반복 크래시 감지** — 기동 직후 반복 실패하는 노드에 bad health 플래그        | N회/M분 재시작 시 플래그                 |
...
| FR-WL-03 | 그룹별 큐 대기시간 p50/p95                                               | 값 표시        |
| FR-WL-04 | **병목 진단** — 제한(softConcurrencyLimit 등)에 걸려 대기 중인 그룹 강조 | 대기 원인 표시 |
| FR-WL-05 | 그룹 클릭 → 해당 그룹의 현재/과거 쿼리 목록 (FR-QUERY-HISTORY 연동)      | 조인 조회 동작 |
| FR-WL-06 | 그룹별 리소스 소비 랭킹                                                  | 정렬 가능      |
...
| ID       | 내용                                           | AC                  |
| -------- | ---------------------------------------------- | ------------------- |
| FR-PT-01 | SSO 인증 (기존 LDAP/AD 연동)                   | 로그인 동작         |
| FR-PT-02 | Grafana/Gateway UI/Trino UI/Superset 링크 허브 | 각 링크 이동        |
...
| 릴리스  | 포함                                                                                                    | 목표                                             |
| ------- | ------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| **R1**  | FR-PORTAL, FR-QUERY-HISTORY, **FR-QUERY-LIVE**, FR-CLUSTER-HEALTH, FR-AUDIT-ACTION, **FR-LOG-DEEPLINK** | "지금 무슨 일이 일어나고 있고, 누가 무엇을 했나" |
