...
| **`/health` 가 헬스 화면을 가렸다** | 콘솔이 `/` 를 갖게 되자 서버의 liveness 프로브(`deploy.md` 에 문서화돼 있다)가 이겼다. 브라우저로 헬스 화면을 열면 `{"status":"ok"}` 가 나왔다. 화면을 **`/cluster-health`** 로 옮기고, `test_ui_mount.py` 가 라우터에서 화면 목록을 읽어 서버 라우트와 겹치는지 대조한다 |
