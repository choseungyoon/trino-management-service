...
그룹 수집은 `/v1/jmx/mbean` 열거 1회 + 그룹 수만큼 MBean 조회다. 그룹이 20개면 폴마다 21회 요청이다. 이를 5초 주기로 돌리면 **코디네이터 부하가 R1 대비 몇 배가 된다.** NFR-PERF-03 은 아직 프로덕션 실측 전이고(`NEXT_STEPS.md` W-1), 현재 수치 0.60%p 는 하한이다. **측정 전에 부하를 늘리지 않는다.**
...
| FR-BM-02 (CPU/Network/Disk 시계열) | Prometheus 미구축 (`NEXT_STEPS.md` W-6) |
